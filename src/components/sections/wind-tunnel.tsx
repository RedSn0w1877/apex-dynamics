...
function BarRow({ label, cd, color }: { label: string; cd: number; color: string }) {
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between font-mono text-[11px] uppercase tracking-[0.16em] text-chalk/60">
        <span>{label}</span>
        <span className="text-chalk">Cd {cd.toFixed(2)}</span>
      </div>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            <h2 className="font-display text-4xl italic font-black uppercase leading-[0.9] tracking-tighter text-chalk md:text-5xl">
              Less drag, <span className="text-volt">less debt.</span>
            </h2>
            <p className="mt-5 max-w-sm text-[15px] leading-relaxed text-chalk/65">
              A lower forefoot rocker and a narrower toe box cut frontal area. Figures below come from a scale-model
              tunnel run at three race-relevant wind speeds.
            </p>
          </Reveal>
